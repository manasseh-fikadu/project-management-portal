...
import { buildReportingTemplateFilename, renderReportingTemplateExcel } from "@/lib/reports/template-excel";
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (format !== "excel") {
      return NextResponse.json({ error: "Template workbooks currently support Excel export only" }, { status: 400 });
    }
    ...
    const buffer = await renderReportingTemplateExcel(data);
    return new NextResponse(new Uint8Array(buffer), {
      status: 200,
      headers: {
        "Content-Type": "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        "Content-Disposition": `attachment; filename="${buildReportingTemplateFilename(data)}"`,
      },
    });
  } catch (error) {
    console.error("Error generating reporting template workbook:", error);
